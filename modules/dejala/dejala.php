...
class Dejala extends CarrierModule
{
    ...
    public function __construct ()
    {
        ...
        $this->id_lang = (! isset($cookie) or ! is_object($cookie)) ? (int) (Configuration::get(
        'PS_LANG_DEFAULT')) : (int) ($cookie->id_lang);
        $this->wday_labels = array($this->l('Sunday'), $this->l('Monday'), 
        $this->l('Tuesday'), $this->l('Wednesday'), $this->l('Thursday'), 
        $this->l('Friday'), $this->l('Saturday'));
        parent::__construct();
        // The parent construct is required for translations
        $this->page = basename(__FILE__, '.php');
        $this->displayName = $this->l('Dejala.com : Courier delivery');
        $this->description = $this->l(
        'Lets Dejala.com handle your deliveries by courier');
        // load configuration only if installed
        if ($this->id) {
            if (true !== extension_loaded('curl')) {
                $this->warning = $this->l(
                'The Dejala module requires php extension cURL to function properly. Please install the php extension "cURL"');
            }
            $this->dejalaConfig = new DejalaConfig();
            $this->dejalaConfig->loadConfig();
            // Update table schema
            if (! isset($this->dejalaConfig->internal_version) ||
             $this->dejalaConfig->internal_version < $this->internal_version) {
                $this->unregisterHook('cart');
                $res = Db::getInstance()->ExecuteS(
                'SELECT * FROM `' . _DB_PREFIX_ . 'dejala_cart` LIMIT 1');
                if ($res) {
                    if (! array_key_exists('cart_date_upd', (int) $res[0])) {
                        Db::getInstance()->Execute(
                        'ALTER TABLE `' . _DB_PREFIX_ .
                         'dejala_cart` ADD COLUMN cart_date_upd DATETIME DEFAULT 0;');
                    }
                    if (! array_key_exists('delivery_price', (int) $res[0])) {
                        Db::getInstance()->Execute(
                        'ALTER TABLE `' . _DB_PREFIX_ .
                         'dejala_cart` ADD COLUMN delivery_price FLOAT DEFAULT NULL;');
                    }
                }
                $this->dejalaConfig->internal_version = $this->internal_version;
    ...
    public function install ()
    {
        if (! file_exists(dirname(__FILE__) . '/' . self::INSTALL_SQL_FILE))
            return (false);
        elseif (! $sql = file_get_contents(
        dirname(__FILE__) . '/' . self::INSTALL_SQL_FILE))
            return (false);
        ...
        if (parent::install() == false or
         ...
         $this->registerHook('processCarrier') == false)
            return (false);
        $this->dejalaConfig = new DejalaConfig();
    ...
    public function uninstall ()
    {
        ...
        if (Configuration::get('PS_CARRIER_DEFAULT') == (int) ($djlCarrier->id)) {
            ...
            false, NULL, Carrier::PS_CARRIERS_AND_CARRIER_MODULES_NEED_RANGE);
            foreach ($carriers as $carrier) {
                if ($carrier['active'] and ! $carrier['deleted'] and
        ...
        if (! parent::uninstall() or ! $this->unregisterHook(
        'updateOrderStatus') or ! $this->unregisterHook('extraCarrier') or
    ...
    public function _postValidation ()
    {
        ...
        if ($method == 'signin') {
            ...
            $responseArray = $djlUtil->getStoreProducts($this->dejalaConfig, 
            $products);
            ...
            foreach ($_POST as $key => $value) {
                if (0 === strpos($key, 'margin_')) {
                    ...
                    if ($product) {
                        ...
                        $public_price = round($product['price'] * $vat_factor, 
                        2);
    ...
    public function _postProcess ()
    {
        ...
        if ($method == 'signin') {
            ...
            $this->dejalaConfig->serviceURL = str_replace('.fr', 
            '.' . $this->dejalaConfig->country, $this->dejalaConfig->serviceURL);
            $this->dejalaConfig->sandboxServiceURL = str_replace('.fr', 
            '.' . $this->dejalaConfig->country, 
            $this->dejalaConfig->sandboxServiceURL);
            $storeAttr = array();
            ...
            if ($response['status'] == 200) {
                ...
                if ($response['status'] == 401)
                    $errors[] = $this->l(
                    'An error occurred while authenticating your account on Dejala.com. Your credentials were not recognized.');
                else
            ...
            $response = $djlUtil->createInstantStore($this->dejalaConfig, 
            Tools::getValue('store_name'));
            ...
            elseif ($response['status'] == 403)
                $errors[] = $this->l(
                'Dejala Server cannot be reached by your Prestashop server. This is most likely due to a limit set by your hosting provider. Please contact their technical support and ask if your server is authorized to initiate outbound HTTP connections.');
            else
                $errors[] = $this->l('Unable to process the action.') . '(' .
                 $response['status'] . ')';
            $this->dejalaConfig->loadConfig();
            ...
            if ($response['status'] != 200)
                $errors[] = $this->l(
                'An error occurred while updating contacts');
        } elseif ($method == 'processes') {
            $djlUtil = new DejalaUtils();
            $response = $djlUtil->setStoreProcesses($this->dejalaConfig, $_POST);
            if ($response['status'] != 200)
                $errors[] = $this->l(
                'An error occurred while updating processes');
        } elseif ($method == 'products') {
            $djlUtil = new DejalaUtils();
            ...
            $this->dejalaConfig->trigerringStatuses = htmlentities(
            $trigerringStatuses, ENT_COMPAT, 'UTF-8');
            $this->dejalaConfig->saveConfig();
            $this->dejalaConfig->loadConfig();
            ...
            if ($response['status'] != 200)
                $errors[] = $this->l(
                'An error occurred while updating products');
            $m_attributes['nb_days_displayed'] = htmlentities(
            Tools::getValue('nb_days'), ENT_COMPAT, 'UTF-8');
            $m_attributes['delivery_delay'] = htmlentities(
            Tools::getValue('delivery_delay'), ENT_COMPAT, 'UTF-8');
            $m_attributes['delivery_partial'] = htmlentities(
            Tools::getValue('delivery_partial'), ENT_COMPAT, 'UTF-8');
            $response = $djlUtil->setStoreAttributes($this->dejalaConfig, 
            $m_attributes);
            if ($response['status'] != 200)
                $errors[] = $this->l(
                'An error occurred while updating products');
            ...
            if ($l_active == "visible_limited") {
                ...
                if ($l_active_list == "") {
                    ...
                    $this->dejalaConfig->saveConfig();
                    $errors[] = $this->l(
                    'You must provide at least one email address to restrict Dejala\'s visibility.');
                } else {
                    $this->dejalaConfig->visibility_status = $l_active;
        ...
        if (! empty($_POST)) {
            ...
            if (count($errors))
				...
				' .
                 ...
                 (($method == 'signin' or $method == 'register' or
                 $method == 'golive') ? '<img src="http://www.prestashop.com/modules/dejala.png?pspid=' .
                 urlencode($this->dejalaConfig->login) . '&mode=' .
                 ($this->dejalaConfig->mode == 'TEST' ? 0 : 1) .
                 '" style="float:right" />' : '') . '
				</div>';
            }
        }
    ...
    public function displayForm ()
    {
        ...
        if (true !== extension_loaded('curl')) {
            $errors[] = $this->l(
            'This module requires php extension cURL to function properly. Please install the php extension "cURL" first.');
            $smarty->assign("disabled", 'disabled="disabled"');
        }
        $registered = TRUE;
        if ((0 == strlen($this->dejalaConfig->login)) ||
         (0 == strlen($this->dejalaConfig->password)))
            $registered = FALSE;
        if ($registered) {
            $djlUtil = new DejalaUtils();
            $responsePing = $djlUtil->ping($this->dejalaConfig, 
            $this->dejalaConfig->mode);
            if (200 != $responsePing['status']) {
                if (401 == $responsePing['status'])
                    ...
                    $errors[] = $this->l(
                    'An error occurred while authenticating your account on Dejala.com. This may be due to a temporary network or platform problem. Please try again later or contact Dejala.com');
                unset($_GET['cat']);
                $registered = FALSE;
            }
        }
        ...
        $smarty->assign("moduleConfigURL", 
        'index.php?tab=AdminModules&configure=dejala&token=' . $_GET['token']);
        $smarty->assign("formAction", 
        Tools::safeOutput($_SERVER['REQUEST_URI']));
        $outputMenu = $this->display(__FILE__, 'dejala_menu.tpl');
        if ($currentTab === 'home') {
            $smarty->assign("login", 
            html_entity_decode(Configuration::get('PS_SHOP_EMAIL'), ENT_COMPAT, 
            'UTF-8'));
            if ($registered) {
                $smarty->assign("visibility_status", 
                $this->dejalaConfig->visibility_status);
                $smarty->assign("visible_users_list", 
                $this->dejalaConfig->visible_users_list);
                $smarty->assign("store_login", 
                html_entity_decode($this->dejalaConfig->login, ENT_COMPAT, 
                'UTF-8'));
                ...
                html_entity_decode($shopName, ENT_COMPAT, 'UTF-8'));
            }
            $outputMain = $this->display(__FILE__, 'dejala_home.tpl');
        } elseif ($currentTab === 'contacts') {
            $contacts = array();
            $djlUtil = new DejalaUtils();
            $responseArray = $djlUtil->getStoreContacts($this->dejalaConfig, 
            $contacts);
            if ('200' == $responseArray['status']) {
            ...
            $location = array();
            $djlUtil = new DejalaUtils();
            $responseArray = $djlUtil->getStoreLocation($this->dejalaConfig, 
            $location);
            if ('200' == $responseArray['status']) {
            ...
            $processes = array();
            $djlUtil = new DejalaUtils();
            $responseArray = $djlUtil->getStoreProcesses($this->dejalaConfig, 
            $processes);
            if ('200' == $responseArray['status']) {
            ...
            if ('200' == $responseArray['status']) {
                ...
                foreach ($products as &$product) {
                    $vat_factor = (1 + ($product['vat'] / 100));
                    $product['price_notax'] = number_format($product['price'], 
                    2, '.', '');
                    $product['price'] = number_format(
                    round($product['price'] * $vat_factor, 2), 2, '.', '');
                    $product['public_price'] = number_format(
                    round($product['price'] + $product['margin'], 2), 2, '.', '');
                    $product['public_price_notax'] = number_format(
                    round($product['public_price'] / $vat_factor, 2), 2, '.', '');
                }
            ...
            $djlUtil = new DejalaUtils();
            $deliveries = array();
            $responseArray = $djlUtil->getStoreDeliveries($this->dejalaConfig, 
            $deliveries);
            if ('200' == $responseArray['status']) {
                foreach ($deliveries as &$delivery) {
                    $delivery['creation_date'] = date('d/m/Y', 
                    $delivery['creation_utc']);
                    $delivery['creation_time'] = date('H\hi', 
                    $delivery['creation_utc']);
                    if (isset($delivery['shipping_start_utc'])) {
                        $delivery['shipping_date'] = date('d/m/Y', 
                        $delivery['shipping_start_utc']);
                        $delivery['shipping_start'] = date('H\hi', 
                        $delivery['shipping_start_utc']);
                        $delivery['shipping_stop'] = date('H\hi', 
                        (int) ($delivery['shipping_start_utc']) +
                         3600 * (int) ($delivery['timelimit']));
                    } else {
                    ...
                    if (isset($delivery['delivery_utc'])) {
                        ...
                        $delivery['delivery_utc']);
                        $delivery['delivery_time'] = date('H\hi', 
                        $delivery['delivery_utc']);
                    }
                }
                $smarty->assign('formAction', 
            ...
            $smarty->assign('trigerringStatuses', 
            $this->dejalaConfig->trigerringStatuses);
            $outputMain = $this->display(__FILE__, 
            'dejala_technical_options.tpl');
        }
        $outputErr = '';
        if (count($errors))
            foreach ($errors as $err)
    ...
    function smartyfyStoreAttributes ()
    {
        ...
        $djlUtil = new DejalaUtils();
        $storeAttrs = array();
        $response = $djlUtil->getStoreAttributes($this->dejalaConfig, 
        $storeAttrs);
        if (200 != $response['status'])
            $errors[] = $this->l(
            'An error occurred while getting store, please try again later or contact Dejala.com');
        else {
            ...
            if ('200' == $responsePing['status'])
                ...
                if (isset($storeAttrs['attributes']) &&
                 isset($storeAttrs['attributes']['request_live']) &&
                 ($storeAttrs['attributes']['request_live'] == 'true'))
    ...
    function displayDeliveryOptions ()
    {
		...
		*/
        ...
        if ($response['status'] == 200) {
            $smarty->assign('nb_days', 
            $store['attributes']['nb_days_displayed']);
            $smarty->assign('delivery_delay', 
            $store['attributes']['delivery_delay']);
            if (isset($store['attributes']['delivery_partial']))
                $smarty->assign('delivery_partial', 
                $store['attributes']['delivery_partial']);
        }
        $wday_selected = array(1, 1, 1, 1, 1, 1, 1);
        $smarty->assign('timetable_css', 
        _MODULE_DIR_ . $this->name . '/timetable.css');
        $smarty->assign("timetable_js", 
        _MODULE_DIR_ . $this->name . '/timetable.js');
        ...
        if ($response['status'] == 200) {
            ...
            $smarty->assign("timetableTpl", 
            dirname(__FILE__) . "/dejala_picking_timetable.tpl");
        }
        $output = $output .
         $this->display(__FILE__, 'dejala_delivery_options.tpl');
        return ($output);
    }
    /**
    ...
    public function hookExtraCarrier ($params)
    {
        ...
        if ($this->dejalaConfig->visibility_status == "invisible")
            return;
        if (($this->dejalaConfig->visibility_status == "visible_limited") &&
         ((int) ($cookie->id_customer) > 0)) {
            $customer = new Customer((int) ($cookie->id_customer));
            if (! in_array($customer->email, 
            preg_split("/[\s,]+/", $this->dejalaConfig->visible_users_list)))
                return;
        }
        $djlUtil = new DejalaUtils();
        $responseGetStore = $djlUtil->getStoreAttributes($this->dejalaConfig, 
        $store);
        ...
        $acceptPartial = true;
        if (! isset($store['attributes']) ||
         ! isset($store['attributes']['delivery_partial']) ||
         ($store['attributes']['delivery_partial'] != '1'))
            $acceptPartial = false;
        if (($isCartOutOfStock == '1') && ! $acceptPartial)
            return;
        $electedProduct = $this->getDejalaProduct($cart);
        // Get id zone
        if (isset($cart->id_address_delivery) and
         $cart->id_address_delivery)
            $id_zone = (int) Address::getZoneById(
        ...
        $storeCalendar = array();
        $calendar = array();
        $response = $djlUtil->getStoreCalendar($this->dejalaConfig, 
        $storeCalendar);
        ...
        if ($response['status'] == 200) {
            foreach ($storeCalendar['entries'] as $weekday => $calEntry) {
                if (isset($productCalendar[$weekday])) {
                    ...
                    $calendar[$weekday]["start_hour"] = max(
                    (int) ($productCalendar[$weekday]["start_hour"]), 
                    (int) ($calEntry["start_hour"]));
                    // MFR090831 - manage picking time : the store is open to (stop_hour - picking time)
                    $calendar[$weekday]["stop_hour"] = min(
                    (int) ($productCalendar[$weekday]["stop_hour"] - 1), 
                    (int) ($calEntry["stop_hour"] - $pickingtime));
                    if ($calendar[$weekday]["stop_hour"] <
                     $calendar[$weekday]["start_hour"])
        ...
        $all_exceptions = array_merge($storeCalendar['exceptions'], 
        $electedProduct['calendar']['exceptions']);
        $dateUtc = $calUtils->getNextDateAvailable(time(), $calendar, 
        $all_exceptions);
        if ($dateUtc == NULL)
            return;
        if ($deliveryDelay > 0) {
            ...
            $dateUtc = $calUtils->addDelay($dateUtc, $deliveryDelay, $calendar, 
            $all_exceptions);
        }
        if ($dateUtc == NULL)
            return;
        ...
        $this->mylog(
        "starting date=" . $this->logValue(date("d/m/Y - H:i:s", $dateUtc), 1));
        $today = getDate();
        $ctime = time();
        ...
        do {
            $wd = date("w", $balladUtc);
            if ((int) ($calendar[$wd]['stop_hour']) <
             (int) ($calendar[$wd]['start_hour']))
                continue;
            $dates[$iDate]['value'] = date("Y/m/d", $balladUtc);
            $dates[$iDate]['ts'] = $balladUtc;
            $dates[$iDate]['label'] = $this->wday_labels[$wd] . " " .
             date("j", $balladUtc);
            ...
            $balladUtc = mktime(0, 0, 0, date('m', $balladUtc), 
            date('d', $balladUtc), date('Y', $balladUtc));
            $balladUtc = $calUtils->getNextDateAvailable($balladUtc, $calendar, 
            $all_exceptions);
            $iDate ++;
        ...
        if ((int) ($dates[0]['stop_hour']) > $now &&
         (int) ($dates[0]['start_hour']) < $now)
            $dates[0]['start_hour'] = $now;
        elseif ((int) ($dates[0]['ts']) == $now &&
         (int) ($dates[0]['stop_hour']) < $now)
            array_shift($dates);
        ...
        $row['delay'] = $this->l('When you want... Dispatch rider') . ', ' .
         $electedProduct['timelimit'] . 'H';
        $row['price'] = $cart->getOrderShippingCost($djlCarrier->id);
        $row['price_tax_exc'] = $cart->getOrderShippingCost($djlCarrier->id, 
        false);
        $row['img'] = _MODULE_DIR_ . $this->name . '/dejala_carrier.gif';
        $resultsArray[] = $row;
        $smarty->assign('carriers', $resultsArray);
        $smarty->assign('my_carrier_selected', 
        (isset($cart->id_carrier) && $cart->id_carrier == $djlCarrier->id));
        $smarty->assign('product', $electedProduct);
        $djlCart = new DejalaCart($cart->id);
        $setDefaultDate = TRUE;
        if ($djlCart && isset($djlCart->shipping_date) &&
         ! empty($djlCart->shipping_date)) {
            ...
            $this->mylog(
            "shipping_date=" . $this->logValue($deliveryDateSelected));
        ...
        if (! $isCartOutOfStock) {
            ...
            $smarty->assign('nostock_info', 
            $this->l(
            'I will select my shipping date when my product is available.'));
    ...
    public function displayInfoByCart ($id_cart)
    {
        ...
        if ($djlCart && isset($djlCart->id_dejala_product) &&
         isset($djlCart->shipping_date)) {
            ...
            if (! empty($mShippingDate) && ($mShippingDate != 0))
                echo $this->l('Shipping date selected') . ' : ' .
                 date('d/m/Y', $mShippingDate) . ', ' . $this->l('starting at') .
                 ' : ' . date('H\hi', $mShippingDate) . '<br/>';
            else
                echo $this->l('Shipping date not yet selected by the customer') .
                 '<br/>';
            if (($djlCart->id_delivery) &&
             Validate::isUnsignedId($djlCart->id_delivery)) {
                $l_delivery = array();
                $l_delivery['id'] = $djlCart->id_delivery;
                $djlUtil = new DejalaUtils();
                $response = $djlUtil->getDelivery($this->dejalaConfig, 
                $l_delivery, $djlCart->mode);
                if ($response['status'] == 200) {
                    if ($l_delivery && $l_delivery['status'] &&
                     $l_delivery['status']['labels'] &&
                     $l_delivery['status']['labels'][Language::getIsoById(
                    (int) $this->id_lang)])
                        echo $this->l('Order') . ' ' .
                         $l_delivery['status']['labels'][Language::getIsoById(
                        (int) $this->id_lang)] . '<br/>';
    ...
    public static function wtf ($var, $arrayOfObjectsToHide = null, $fontSize = 11)
    {
        ...
        '<span style="color: #079700;">$1</span>$2', $text);
        ...
        $text);
        echo '<pre style="font-size: ' . $fontSize . 'px; line-height: ' .
         $fontSize . 'px;text-align:left;">' . $text . '</pre>';
    }
    public function hookProcessCarrier ($params)
    {
        ...
        if (! empty($dejalaCarrierID) && ! empty($carrierID) &&
         ((int) ($dejalaCarrierID) == (int) ($carrierID))) {
            ...
            $date_shipping = 'NULL';
            if (isset($_POST['shipping_day']) and ! empty(
            $_POST['shipping_day']) and (10 <= strlen($_POST['shipping_day']))) {
                ...
                $shippingTime = mktime($shippingHour, 0, 0, $ship_month, 
                $ship_day, $ship_year);
    ...
    public function hookUpdateOrderStatus ($params)
    {
        ...
        $triggeringStatusList = html_entity_decode(
        Configuration::get('DJL_TRIGERRING_STATUSES'), ENT_COMPAT, 'UTF-8');
        $this->mylog("triggeringStatusList=" . $triggeringStatusList);
        $triggeringStatuses = explode(",", $triggeringStatusList);
        $orderID = $params["id_order"];
        if ((NULL !== $orderID) &&
         (TRUE === in_array($currentOrderStatusID, $triggeringStatuses))) {
    ...
    public function placeOrder ($mOrder)
    {
        ...
        if (! $djlCart->id_delivery) {
            ...
            $djlUtil = new DejalaUtils();
            $response = $djlUtil->orderDelivery($this->dejalaConfig, $delivery, 
            $djlCart->mode);
            ...
            if ("201" === $statusCode) {
                ...
                if (Validate::isUnsignedId($delivery['id'])) {
                    $this->mylog(
                    "updating dejala cart id_delivery=" . $delivery['id']);
                    $djlCart->id_delivery = $delivery['id'];
                    $djlCart->update();
                }
                if (is_null($mOrder->shipping_number) ||
                 (0 === strlen($mOrder->shipping_number))) {
                    $this->myLog(
                    'setting Order->shipping_number to ' .
                ...
                $this->myLog(
                "NOK - Problem sending Order to dejala.com");
            }
        }
    }
    ...
    private function getDejalaProductPrice ($cart)
    {
        ...
        if (isset($djlCart->delivery_price) &&
         $cart->date_upd <= $djlCart->cart_date_upd)
    ...
    private function getDejalaProduct ($cart, $productId = -1)
    {
        ...
        $djlCart = $this->getDejalaCart($cart->id);
        if (isset($djlCart->delivery_price) &&
         $cart->date_upd <= $djlCart->cart_date_upd && isset($djlCart->product))
        ...
        $quotation["price"] = $cart->getOrderTotal(true, 
        ...
        $products = array();
        $responseArray = $djlUtil->getStoreQuotation($this->dejalaConfig, 
        $quotation, $products);
        if ($responseArray['status'] != '200')
            return $shipping_cost;
        $this->mylog(
        "found quotation=" . $this->logValue($responseArray['response'], 1));
        $electedProduct = NULL;
        foreach ($products as $key => $product)
            if (is_null($electedProduct) ||
             ((int) ($electedProduct['priority']) > (int) ($key)))
        ...
        $priceTTC = round(
        ($electedProduct['price'] * $vat_factor) + $electedProduct['margin'], 2);
    ...
    public static function getHttpHost ($http = false, $entities = false)
    {
        ...
        if ($http)
            $host = (Configuration::get('PS_SSL_ENABLED') ? 'https://' : 'http://') .
             $host;
        return $host;
    }
}
