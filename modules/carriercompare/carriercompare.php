...
class CarrierCompare extends Module
{
    public function __construct ()
    {
        ...
        'Module to compare carrier possibilities before using  the checkout process');
    }
    public function install ()
    {
        if (! parent::install() or ! $this->registerHook('shoppingCart') or
    ...
    public function hookShoppingCart ($params)
    {
        ...
        if (! $this->isModuleAvailable())
            return;
        $smarty->assign(
        array('countries' => Country::getCountries((int) $cookie->id_lang), 
        'id_carrier' => ($params['cart']->id_carrier ? $params['cart']->id_carrier : Configuration::get(
        'PS_CARRIER_DEFAULT')), 
        'id_country' => (isset($cookie->id_country) ? $cookie->id_country : Configuration::get(
        'PS_COUNTRY_DEFAULT')), 
	...
	*/
    ...
    public function saveSelection ($id_country, $id_state, $zipcode, $id_carrier)
    {
        ...
        if (! Validate::isInt($id_country) ||
         ! Validate::isLoadedObject(new Country($id_country)))
        ...
        if (! Validate::isInt($id_carrier) ||
         ! Validate::isLoadedObject(new Carrier($id_carrier)))
        ...
        if (! in_array($id_carrier, $ids_carrier))
            $errors[] = $this->l(
            'This carrier ID isn\'t available for your selection');
        if (sizeof($errors))
            return $errors;
        $cookie->id_country = $id_country;
        $cookie->id_state = $id_state;
	...
	*/
    private function checkZipcode ($zipcode, $id_country)
    {
        ...
        if (! $zipcodeFormat)
            return false;
        $regxMask = str_replace(array('N', 'C', 'L'), 
        array('[0-9]', Country::getIsoById((int) $id_country), '[a-zA-Z]'), 
        $zipcodeFormat);
