...
class Cheque extends PaymentModule
{
    ...
    public function __construct ()
    {
        ...
        array('CHEQUE_NAME', 'CHEQUE_ADDRESS'));
        ...
        if (! isset($this->chequeName) or ! isset($this->address))
            $this->warning = $this->l(
            '\'To the order of\' and address must be configured in order to use this module correctly.');
    ...
    {
        if (! Configuration::deleteByName('CHEQUE_NAME') or
         ! Configuration::deleteByName('CHEQUE_ADDRESS') or ! parent::uninstall())
            return false;
        return true;
    }
    private function _postValidation ()
    {
        if (Tools::isSubmit('btnSubmit')) {
            if (! Tools::getValue('name'))
                ...
                '\'To the order of\' field is required.');
    ...
    private function _postProcess ()
    {
        if (Tools::isSubmit('btnSubmit')) {
            ...
            Configuration::updateValue('CHEQUE_ADDRESS', 
            Tools::getValue('address'));
        }
        $this->_html .= '<div class="conf confirm"><img src="../img/admin/ok.gif" alt="' .
         $this->l('OK') . '" /> ' . $this->l('Settings updated') . '</div>';
    }
    private function _displayCheque ()
    {
        $this->_html .= '<img src="../modules/cheque/cheque.jpg" style="float:left; margin-right:15px;"><b>' .
         $this->l('This module allows you to accept payments by check.') .
         '</b><br /><br />
		' .
         $this->l(
        'If the client chooses this payment mode, the order status will change to \'Waiting for payment\'.') .
         '<br />
		' .
         $this->l(
        'Therefore, you will need to manually confirm the order as soon as you receive a check.') .
         '<br /><br /><br />';
    }
        ...
        $this->_html .= '<form action="' .
         ...
         '</legend>
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					<tr><td colspan="2">' .
         $this->l(
        'Please specify the name and address to which customers must send their check.') .
         '.<br /><br /></td></tr>
					<tr><td width="130" style="height: 35px;">' .
         $this->l('To the order of') .
         '</td><td><input type="text" name="name" value="' .
         Tools::htmlentitiesUTF8(Tools::getValue('name', $this->chequeName)) . '" style="width: 300px;" /></td></tr>
					<tr>
						<td width="130" style="vertical-align: top;">' .
         $this->l('Address') .
         '</td>
						<td><textarea name="address" rows="3" cols="53">' .
         Tools::htmlentitiesUTF8(Tools::getValue('address', $this->address)) .
         '</textarea></td>
					</tr>
    ...
    public function execPayment ($cart)
    {
        ...
        $smarty->assign(
        array('nbProducts' => $cart->nbProducts(), 
        'cust_currency' => $cart->id_currency, 
        'currencies' => $this->getCurrency((int) $cart->id_currency), 
        'total' => $cart->getOrderTotal(true, Cart::BOTH), 
        'isoCode' => Language::getIsoById((int) ($cookie->id_lang)), 
        'chequeName' => $this->chequeName, 
        'chequeAddress' => nl2br2($this->address), 'this_path' => $this->_path, 
    ...
    public function hookPaymentReturn ($params)
    {
        ...
        if ($state == Configuration::get('PS_OS_CHEQUE') or
         $state == Configuration::get('PS_OS_OUTOFSTOCK'))
            $smarty->assign(
            array(
            'total_to_pay' => Tools::displayPrice($params['total_to_pay'], 
            $params['currencyObj'], false), 'chequeName' => $this->chequeName, 
            'chequeAddress' => nl2br2($this->address), 'status' => 'ok', 
